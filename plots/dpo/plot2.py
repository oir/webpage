...
def log_sigmoid(x: float, beta: float) -> float:
    return np.log(1 / (1 + np.exp(-x * beta)))
...
xname= "$(\\log \\pi_\\theta(y_w \\mid x) - \\log \\pi_\\theta(y_l \\mid x)) - (\\log \\pi_\\text{ref}(y_w \\mid x) - \\log \\pi_\\text{ref}(y_l \\mid x))$"
yname= "$-\\log (\\sigma ( \\beta \cdot  ) )$"

x = np.linspace(-6, 6, 1000)
betas = [10, 3.16, 1, 0.316, 0.1]
data = pd.DataFrame({
    xname: np.tile(x, len(betas)),
    yname: np.concatenate([-log_sigmoid(x, beta) for beta in betas]),
